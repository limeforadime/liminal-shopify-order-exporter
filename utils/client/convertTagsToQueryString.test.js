...
import convertTagsToQueryString, {
  convertTagsStrategies,
  dateSubStrategies,
} from './convertTagsToQueryString';
const { paymentStrategy, statusStrategy, fulfillmentStrategy } = convertTagsStrategies;

let allStatusChoices;
let paymentStatusChoices, statusChoices, fulfillmentStatusChoices;
...
beforeEach(() => {
  ...
  ({ paymentStatusChoices, statusChoices, fulfillmentStatusChoices } = allStatusChoices);
});
describe('Main convertTagsToQueryString function', () => {
  it('Should return a string', () => {
    expect(typeof convertTagsToQueryString(['Date: On Nov 12, 2021'], moment, allStatusChoices)).toBe(
      'string'
    );
    expect(
      typeof convertTagsToQueryString(['Date: Before Nov 12, 2021', 'Status: Open'], moment, allStatusChoices)
    ).toBe('string');
    expect(typeof convertTagsToQueryString(['Date: After Nov 12, 2021'], moment, allStatusChoices)).toBe(
      'string'
    );
    expect(
...
describe('Date', () => {
  ...
  describe('"After" strategy', () => {
    it('Basic input test', () => {
      expect(dateSubStrategies.after('Nov 12, 2021', moment)).toBe(
        'created_at_min=2021-11-13T07%3A59%3A59.999Z'
      );
    });
  });
  ...
  describe('"Before" strategy', () => {
    it('Basic input test', () => {
      expect(dateSubStrategies.before('Nov 12, 2021', moment)).toBe(
        'created_at_max=2021-11-12T08%3A00%3A00.000Z'
      );
    });
  });
});
...
describe('Fulfillment status', () => {
  it('Basic input test', () => {
    expect(
      fulfillmentStrategy('Shipped, Partially fulfilled, Unfulfilled', fulfillmentStatusChoices)
    ).toEqual('fulfillment_status=shipped%2Cpartial%2Cunfulfilled');
